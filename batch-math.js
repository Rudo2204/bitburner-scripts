...
export function calculateDelays(ns, target) {
	...
	var serv = ns.getServer(target);
	serv.hackDifficulty = serv.minDifficulty;
	const hack_time = ns.formulas.hacking.hackTime(serv, ns.getPlayer());
	const grow_time = ns.formulas.hacking.growTime(serv, ns.getPlayer());
	const weak_time = ns.formulas.hacking.weakenTime(serv, ns.getPlayer());
